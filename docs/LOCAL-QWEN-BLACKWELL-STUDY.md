...
The measured speed penalty is implementation-specific. Q4 and Q8 both used the same host-ring transport, P8 partitioning, direct-hot policy, and pipelined attention path. The difference was inside that ring path: Q4 used the fused K+V Q4-to-F16 conversion, while Q8 used the generic per-type Q8-to-F16 conversion branch because no equivalent fused Q8 conversion kernel had been implemented. Thus, "generic" does not mean that Q8 bypassed the ring or fell back to CPU attention. Q8 with a 65,536-token hot set was measured only on the short prompt. The long Q8 run used 32,768 hot tokens by design, so this study does not claim that Q8 hot-64K produced an OOM. Given the clear long-context cost and absent demonstrated quality benefit, the frozen profile retains Q4_0 for target and draft KV.
